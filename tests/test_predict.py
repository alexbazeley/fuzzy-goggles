...
class TestPredictPassage:
    @pytest.fixture
    def mock_model_v2(self, tmp_path):
        """Create a minimal v2 model weights file."""
        n = len(FEATURE_NAMES)
        model = {
            "version": 2,
            "weights": [0.1] * n,
    ...
    def test_prediction_with_model(self, sample_bill, mock_model_v2):
        """predict_passage returns full result dict with model loaded."""
        import legislator.model.predict as predict_mod
        old_cache = predict_mod._model_cache
        predict_mod._model_cache = mock_model_v2
        try:
            result = predict_passage(sample_bill)
